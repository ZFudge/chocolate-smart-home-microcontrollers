...
byte maxCount = 50;
byte actualNumOfPixelObjects = 0;
...
void init(const byte dataPin, const byte numOfPixels, neoPixelType npType = NEO_GRB + NEO_KHZ800) {
    ...
    // with the pattern ending when n+maxCount*index exceeds actualNumOfPixelObjects
    this->numOfPixels = numOfPixels;
    this->actualNumOfPixelObjects = min(numOfPixels, maxCount);
    ...
    pixels = new Pixel[actualNumOfPixelObjects];
    if (transform)
        for (byte i = 0; i < actualNumOfPixelObjects; i++)
...
void loop() {
    ...
    // Twinkle / Transform
    byte ACTIVE_TRANSFORM_CYCLES_REMAINING = actualNumOfPixelObjects;
    for (byte i = 0; i < actualNumOfPixelObjects; i++) {
        Pixel *pixel = &pixels[i];
...
void setMaxCount(const byte maxCount) {
    if (maxCount == 0 || this->actualNumOfPixelObjects > 0) return;
...
void settleAnyTransforms() {
    ...
    been turned off. */
    byte ACTIVE_CYCLES_REMAINING = actualNumOfPixelObjects;
    for (byte i = 0; i < actualNumOfPixelObjects; i++) {
        Pixel *pixel = &pixels[i];
...
void applyPixelSettingsToNeoPixel(const byte pixelIndex, const Pixel *pixel) {
    ...
    for (int i = pixelIndex; i < (int)numOfPixels; i += this->maxCount)
